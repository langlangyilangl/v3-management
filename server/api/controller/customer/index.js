...
exports.getSomeCustomerInfo = (req, res) => {
  ...
  const { tel, customerName, page, size } = req.body;
  const beforeSize = (page - 1) * size; //前面多少条
  const sql = `SELECT * FROM (select * from customer where tel like ? and customerName like ?) as A  ORDER BY customerId LIMIT ?,?;`;
  db.query(sql, [`%${tel ?? ''}%`, `%${customerName ?? ''}%`, beforeSize, size], (err, result) => {
    if (err) {
      return res.cc(err);
    }
